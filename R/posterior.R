#' Get Principal Direction in Polar Coordinates
#'
#' @param angles A vector of angles in radians.
#' @param return_angle A logical value indicating whether to return the angle of the principal direction
#'
#' @return A vector representing the principal direction in Cartesian coordinates, or the angle of the principal direction if `return_angle` is TRUE.
#' @export
get_principal_direction <- function(angles, return_angle = FALSE) {
  ...
  # Eigen decomposition
  eig <- eigen(M)
  direction <- eig$vectors[, which.max(eig$values)]

  if (return_angle) {
    return(atan2(direction[2], direction[1]))
  } else {
    return(direction)
  }
}
#' Align Angles to a Given Direction
#'
#' @param angles A vector of angles in radians.
#' @param direction A vector representing the direction to which angles should be aligned.
#'
#' @return A vector of angles aligned to the specified direction.
#' @export
align_angles_to_direction <- function(angles, direction) {
  # Convert angles to unit vectors
  samples <- cbind(cos(angles), sin(angles))

  # Flip vectors that point opposite to the direction
  aligned <- t(apply(samples, 1, function(v) {
    if (sum(v * direction) < 0) return(-v) else return(v)
  }))

  # Convert aligned vectors back to angles in [-pi, pi]
  aligned_angles <- atan2(aligned[,2], aligned[,1])

  return(aligned_angles)
}
#' Align Angles to Principal Direction
#'
#' This function aligns a set of angles to the principal direction derived from the angles.
#' Since we estimate the mean direction from the samples generated by Bingham distribution,
#' it is necessary to calculate the principal direction. This would mitigate the problem with
#' bimodal distribution of angles, which is by the nature of the Bingham distribution.
#' In case of single mode distribution, the principal direction is the sample average.
#' To ease the interpretation, the angles are aligned to the principal direction.
#'
#' @param angles A vector of angles in radians.
#'
#' @return A vector of angles aligned to the principal direction.
#' @export
align_to_principal_direction <- function(angles) {

  # Get principal direction
  principal_direction <- get_principal_direction(angles)

  # Align angles to the principal direction
  aligned_angles <- align_angles_to_direction(angles, principal_direction)
...
make_posterior_summary <- function(stan_fit, issue_label = NULL, rc_label = NULL, legis_label = NULL, missing_label = NULL) {
  ...
  theta_samples <- apply(theta_samples, 2, align_to_principal_direction)

  u_samples <- extract(stan_fit, "u")$u
  u_samples <- apply(u_samples, 2, align_to_principal_direction)
  ...
  theta_antipodal <- theta_df |>
    filter(.data$parameter == "theta") |>
    mutate(parameter = "theta_antipodal")
  theta_aux <- theta_df |>
    filter(.data$parameter != "theta")
  theta <- process_data(theta_samples, "theta", "issue_index", theta_antipodal$issue_label)
  ...
  u_antipodal <- u_df |>
    filter(.data$parameter == "u") |>
    mutate(parameter = "u_antipodal")
  u_aux <- u_df |>
    filter(.data$parameter != "u")
  u <- process_data(u_samples, "u", "rc_index", u_antipodal$rc_label)
  ...
  res <- list(
    theta = theta,
    theta_antipodal = theta_antipodal, theta_aux = theta_aux,
    u = u,
    u_antipodal = u_antipodal, u_aux = u_aux,
    x = x,
    other_bill = other_bill,
    y_imputed = y_imputed
  )
