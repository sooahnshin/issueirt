...
#' Recode Roll Call Votes (Optional)
#'
#' This function preprocesses roll call vote data by recoding votes to prevent sign flipping.
#' It is designed to work with matrices of votes, those obtained from `pscl::rollcall` for example.
#' You may use this only when you fit the model using von Mises-Fisher distribution
#' instead of Bingham distribution (default).
#'
#' @param votes A matrix of votes (output of `pscl::rollcall(...)$votes`; recommended to use `filter_votes` first).
...
#' Fix sign-flip based on the fit (Optional)
#'
