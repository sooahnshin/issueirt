...
#' @useDynLib issueirt, .registration=TRUE
#' @export
issueirt_dynamic_stan <- function(
    data,
    init,
    chains = 2,
    iter = 10,
...
#' Run Stan Model with Fixed Sign
#'
#' This function runs a Stan model using the `rstan::sampling` function to
#' estimate issue specific ideal points.
#' It allows for specifying various parameters for the Stan model execution.
#' It uses von Mises-Fisher distribution where the yea-to-nay direction aligns with
...
issueirt_vmf_stan <- function(
    ...
    ...) {
  stan_fit <- rstan::sampling(
    object = stanmodels$issueirt_vmf,
    data = data,
    init = init,
    chains = chains,
    iter = iter,
...
#' Run Stan Model with Fixed Sign (dynamic)
#'
#' This function runs a Stan model using the `rstan::sampling` function to
#' estimate issue specific ideal points.
#' It allows for specifying various parameters for the Stan model execution.
...
#' @useDynLib issueirt, .registration=TRUE
#' @export
issueirt_dynamic_vmf_stan <- function(
    data,
    init,
    chains = 2,
    iter = 10,
