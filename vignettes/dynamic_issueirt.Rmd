...
This vignette provides a tutorial on how to use the dynamic extension of the `IssueIRT` model.
Please refer to the `vignette("using_issueirt")` for the basic usage of the `IssueIRT` model.
Further details on the dynamic model can be found in @issueirt.
...
For users who want to quickly fit a dynamic IssueIRT model, the `issueirt_dynamic()` function provides a one-shot wrapper that handles all preprocessing, model fitting, and post-processing automatically:

```{r quick_start, eval=FALSE}
library(issueirt)

# Load the 1890s U.S. House data
data("us1890s_votes")
data("us1890s_issue")
data("us1890s_legis")
data("us1890s_bills")

# Prepare issue codes as a list of vectors
...
# View results
summary(fit)
plot(fit, type = "ideal_points")
plot(fit, type = "issue_specific")
```

The rest of this vignette demonstrates the step-by-step workflow that `issueirt_dynamic()` automates, which provides more control over each stage of the analysis.
...
Please refer to the documentation for more details on the data. Also, please refer to `data-raw/us_southern.R` for the data preparation process.

The following code will prepare miscellaneous data for the visualization:
...
We can prepare issue labels using `make_issue_code()` function:
...
We can make dynamic version of `rollcall` object using `make_dynamic_rollcall()` function:
...
Recall that the Bayesian IRT model has an identification problem, and thus we fix $d+1$ legislators' ideal points.
...
const_ls <- find_constraints(ideal, dynamic_rc$rollcall, pol_rc1 = pol_rc1, pol_rc2 = pol_rc2, 
                             ...
                             as_list = TRUE)
const_ls <- lapply(const_ls, function(x) x[c(2, 1)]) # swap the order of dimensions (for the ease of illustration)
print(const_ls)
```
...
## Step 3: Make inputs for the model
...
Finally, we fit the dynamic `IssueIRT` model using the `issueirt_dynamic_stan` function.
...
## script is available at data-raw/us_southern_dynamic.R, which is the same as the code above
fit_dynamic <- issueirt:::fit_dynamic
```

## Step 5: Post-process the model parameters
...
Now, we visualize the estimated parameters using their posterior means.
...
We can also visualize issue-specific axes using the `plot_issueaxis()` function.
...
- Points: multidimensional ideal points
  - small light grey: legislators who were not present in that term
...
We can compute $\hat{R}$ statistics and effective sample size (ESS) using functions from the `posterior` package, for example.
...
One may also want to visualize the posterior distribution of the estimated parameters, especially $\theta$'s, using the `bayesplot` package.

```{r bayes_plot}
samples_pp <- post_process(
  stan_fit = fit_dynamic, constraints = const_dynamic, legis_label = dynamic_stan_input$misc$legis_term
  )
df <- samples_pp$theta |> as.data.frame()
colnames(df) <- dynamic_stan_input$misc$issue_code_df$codebook$label
bayesplot::mcmc_areas(df |> select(contains("Monetary")), prob = 0.8)
```

We may want to adjust the number of chains, warmup, and iterations based on these convergence diagnostics.


## Step 7: Calculate issue-specific ideal points
...
plot_issueirt(issueirt = issue_irt |> filter(issue_label %in% c("H52_Monetary", "H53_Monetary", "H54_Monetary")), 
              ...
              alpha = 0.5, size = 3)
```
You may want to fix the sign-flip of each issue-specific ideal points if needed.
For instance, if the estimated issue-specific ideal points for some issue areas have 
opposite left-to-right direction, you may want to flip the sign of the estimated ideal points for such issue areas.
Here, we flip the sign so that Populists are on the left.

```{r calculate_issue_specific_flip, fig.width=10, fig.height=5}
issue_irt_flip <- issue_irt |>
  mutate(issue_index = as.character(issue_index),
         legis_index = as.character(legis_index)) |>
  mutate(across(where(is.numeric), ~ if_else(issue_label %in% c("H52_Monetary", "H53_Monetary", "H54_Monetary"), -., .)))
plot_issueirt(issueirt = issue_irt_flip |> filter(issue_label %in% c("H52_Monetary", "H53_Monetary", "H54_Monetary")), 
              p.title = "Monetary Policy Specific Ideal Points",
              breaks.group = breaks.group, values.shape = values.shape, values.color = values.color,
              alpha = 0.5, size = 3)
```
