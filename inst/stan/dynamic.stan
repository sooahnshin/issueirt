data {
 ...
 real omega[J]; // Precision for ideal point
 int<lower=1> idx[J]; // Index of prior for legislator-term
}
parameters {
...
transformed parameters {
  row_vector[2] x_ext[J+1]; // Ideal point of legislator j with (0,0) appended at the end
  ...
  for (k in 1:J) {
      x_ext[k] = x[k]; // Copy each row from x to x_ext
    }
  x_ext[J+1] = [0, 0]; // Adding the extra row with zeros
}
model {
  for (k in 1:J) { // Ideal point
    x[k][1] ~ normal(x_ext[idx[k]][1], omega[k]);
    x[k][2] ~ normal(x_ext[idx[k]][2], omega[k]);
  }
